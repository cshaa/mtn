...
pub fn primitive_is_subtype_of(s: Primitive, o: Primitive) -> Bool {
  s == o
}
...
pub fn function_argument_is_subtype_of(
  s: FunctionArgument,
  o: FunctionArgument,
) -> Bool {
  case s, o {
    FunctionArgument(_, contravariant_s), FunctionArgument(_, contravariant_o) ->
      value_type_is_subtype_of(contravariant_o, contravariant_s)
  }
}
...
pub fn function_is_subtype_of(s: Function, o: Function) -> Bool {
  case s, o {
    Function(args_s, ret_s), Function(args_o, ret_o) ->
      value_type_is_subtype_of(ret_s, ret_o)
      // don't allow arity mismatch
      && list.length(args_s) == list.length(args_o)
      && list.zip(args_s, args_o)
      |> list.all(fn(p) { function_argument_is_subtype_of(p.0, p.1) })
  }
}
...
pub fn trait_is_subtype_of(subject: Trait, other: Trait) -> Bool {
  subject == other
  || subject.extends
  |> list.contains(other)
  || subject.extends
  |> list.any(trait_is_subtype_of(_, other))
}
...
pub fn struct_is_subtype_of(subject: Struct, other: Struct) -> Bool {
  subject == other
}
...
pub fn enum_is_subtype_of(subject: Enum, other: Enum) -> Bool {
  subject == other
}
...
pub fn value_type_is_subtype_of(subject: ValueType, other: ValueType) -> Bool {
  case subject, other {
    _, _ if subject == other -> True

    ValueTypeNever, _ -> True

    ValueTypePrimitive(s), ValueTypePrimitive(t) ->
      primitive_is_subtype_of(s, t)
    ValueTypeFunction(s), ValueTypeFunction(t) -> function_is_subtype_of(s, t)
    ValueTypeTrait(s), ValueTypeTrait(t) -> trait_is_subtype_of(s, t)
    ValueTypeStruct(s), ValueTypeStruct(t) -> struct_is_subtype_of(s, t)
    ValueTypeEnum(s), ValueTypeEnum(t) -> enum_is_subtype_of(s, t)
...
pub type Effect {
  EffectUnit(id: UniqueId)
  EffectPayload(id: UniqueId, struct: Struct)
}

pub type EffectTrait {
  EffectTrait(id: UniqueId, extends: List(EffectTrait))
}

// TODO better naming
pub type EffectOrEffectTrait {
  EffectNotEffectTrait(Effect)
  EffectTraitNotEffect(EffectTrait)
}

pub type Type {
  TypeValue(value: ValueType)
  TypeEffect(value: ValueType, effects: List(EffectOrEffectTrait))
}
